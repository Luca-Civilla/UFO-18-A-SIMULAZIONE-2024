...
class View(ft.UserControl):
    ...
    def load_interface(self):
        # title
        self._title = ft.Text("2018 A - Ufo sighting", color="blue", size=24)
        self._page.controls.append(self._title)

        #ROW with some controls
        self._txtAnno = ft.TextField(label="Anno")
        self.btn_anno = ft.ElevatedButton(text="Seleziona Anno", on_click=self._controller.handle_selezione)

        self.ddshape = ft.Dropdown(label="Shape")
        # button for the "creat graph" reply
        self.btn_graph = ft.ElevatedButton(text="Crea Grafo", on_click=self._controller.handle_graph)
        row1 = ft.Row([self._txtAnno, self.btn_anno],
